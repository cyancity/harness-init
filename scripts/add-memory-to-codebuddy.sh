#!/usr/bin/env bash
# 把本机共享记忆 ~/memory 接入任意 coding agent 的全局配置文件。
# 用法：
#   bash add-memory-to-codebuddy.sh [agent名] [目标配置文件]
# 例：
#   bash add-memory-to-codebuddy.sh codebuddy ~/.codebuddy/CODEBUDDY.md
#   bash add-memory-to-codebuddy.sh claude   ~/.claude/CLAUDE.md
# 若不传参，默认 codebuddy / ~/.codebuddy/CODEBUDDY.md。
#
# 逻辑：从已接入的参照 agent（opencode）抽取 <!-- shared-memory --> 块，
#       把其中的 <agent> 占位替换为目标 agent 名，追加到目标文件。
set -euo pipefail

AGENT_NAME="${1:-codebuddy}"
TARGET="${2:-$HOME/.codebuddy/CODEBUDDY.md}"
